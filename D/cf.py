...
class DataProcessor():
    ...
    def compute_value(self, user_id, product_id, user_cluster_no, actions, product_cluster_no=[]):
        """
        評価値を算出するメソッド
    ...
    def make_matrix_for_CF(self,user_cluster_dict,product_cluster_dict=[]):
        ...
        cf_ndarray = np.empty((max_user_id, max_product_id)).astype('float16')
        ...
        for user_id, user_actions in cf_data.items():
            ...
            for product_id, actions in user_actions.items():
                ...
                value = self.compute_value(user_id, product_id, actions, user_cluster_no)
