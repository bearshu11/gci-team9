...
if __name__ == '__main__':
  ...
  print('train読み込み')
  train = pd.read_table('../data/train/train_B.tsv')
  ...
  test_B['user_id_int'] = test_B['user_id'].map(lambda x : user_ids_index.get_loc(x))
  ...
  #イベントごとに重みをかけて足し算
  scoremat = eventcountmats[0] * 6.0 + eventcountmats[1] * 1.0 + eventcountmats[2] * 0.3 + eventcountmats[3] * 8.0
  print(time.time() - start),
  print('seconds')

  print('レコメンドを計算')
  submit_df = sparse_collaborative_filtering(scoremat, test_B, user_ids, product_ids)

  print('提出形式で保存')
  submit_df.to_csv('submit_B.tsv', sep='\t', header=False, index=False, encoding='utf-8', line_terminator='\r\n')
